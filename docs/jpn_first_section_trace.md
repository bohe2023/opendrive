...
本说明以导出的 `out/JPN/map.xodr` 首个 `<laneSection s="0">` 为例，逐条对应到 CSV 源文件的具体行，并说明转换脚本的处理流程，便于对客户解释“从原始表格到最终 OpenDRIVE” 的链路。

## 1. 车道截面在 XODR 中的结果
- `map.xodr` 第 878–919 行展示了首段的车道编排：左侧 3 条行车道（宽度 3.59 m、3.91 m、3.63 m）外加 1 条左路肩 1.391 m，右侧只有一条宽度 0.398 m 的路肩。【F:out/JPN/map.xodr†L878-L919】

## 2. 中心线几何来源
- 对应的经纬度采样点位于 `PROFILETYPE_MPU_LINE_GEOMETRY.csv` 第 2–10 行，`Offset[cm]`=858733 的段落给出了 9 个采样点；这些点和第 1 行表头共同表明列含义为 Offset、End Offset、Lane Number、纬度、经度等。【F:input_csv/JPN/PROFILETYPE_MPU_LINE_GEOMETRY.csv†L1-L10】
- `PROFILE_MPU_MAP_DATA_BASE_POINT.csv` 第 2 行提供了该 Path=285 的基准点（纬度 36.11478232688149°，经度 139.6358160999687°），作为局部坐标系的原点。【F:input_csv/JPN/PROFILE_MPU_MAP_DATA_BASE_POINT.csv†L1-L9】
...
## 3. 车道分段与拓扑
- `PROFILETYPE_MPU_ZGM_LANE_LINK_INFO.csv` 第 1–4 行给出了同一 Offset 范围（858733–861908 cm，长度 31.75 m）内的 3 条行车道：`幅員情報[cm]` 分别为 359、391、363，换算即 3.59 m、3.91 m、3.63 m；`レーン番号` 列标注了 1、2、3；`左側車線のレーンID` / `右側車線のレーンID` 以及 `前方レーンID`/`後方レーンID` 列描述了相邻与前后继关系。【F:input_csv/JPN/PROFILETYPE_MPU_ZGM_LANE_LINK_INFO.csv†L1-L4】
- `make_sections`（`topology/core.py` 第 109–153 行）会把 lane link 与 lane division 中出现的 Offset（经 `offset_mapper` 转换后）加入分割点集合，并与中心线总长一起生成 `[s0, s1)` 车道截面。本例中 `Offset=858733 cm` 映射为 `s0=0`，`End Offset=861908 cm` 映射为 `s1≈31.9456 m`，因此首段长度与 XODR 保持一致。【F:csv2xodr/topology/core.py†L109-L153】
...
## 4. 标线与显式几何
- `PROFILETYPE_MPU_ZGM_LANE_DIVISION_LINE.csv` 第 2–7 行覆盖了同一 `Path Id=285`、`Offset=858733 cm` 段的白线数据：`区画線種別`（列名包含 “種別”）分别给出了标线类型 ID；`始点/終点側線幅[mm]` 提供了标准宽度（532 mm → 0.532 m 等）。【F:input_csv/JPN/PROFILETYPE_MPU_ZGM_LANE_DIVISION_LINE.csv†L1-L7】
...
## 5. 路肩生成逻辑
- `PROFILETYPE_MPU_ZGM_SHOULDER_WIDTH.csv` 第 2–9 行记录了同一 Offset 区段的左右路肩宽度：`Hl[cm]` 为左路肩（例如 274 cm → 2.74 m），`EHl[cm]` 为右路肩（本段为 0 cm）；`Lane Number=7` 与 path 对应。【F:input_csv/JPN/PROFILETYPE_MPU_ZGM_SHOULDER_WIDTH.csv†L1-L9】
- `build_shoulder_profile`（`normalize/core.py` 第 1913–1980 行）把这些宽度去重、转米并按 `[s0,s1)` 聚合，结合 `offset_mapper` 纠偏后得到左 1.390625 m、右 0.398125 m 的平均宽度段。【F:csv2xodr/normalize/core.py†L1913-L1980】
- `apply_shoulder_profile`（`lane_spec.py` 第 1276–1355 行）会在每个 lane section 的左右侧插入 type=`shoulder` 的车道，并继承前后继关系，因此在首段自动生成了 ID=4 与 ID=-1 的路肩，宽度正是上一条计算结果。【F:csv2xodr/lane_spec.py†L1276-L1355】
...
- 车道宽度字段依旧会写进 `<lane><width>`：`build_lane_spec` 会把 lane link 里 359/391/363 cm 等值转成米后塞到 `lane_spec.widths`，`xodr_writer` 再生成 `<width a="3.59">` 这类系数，驱动查看器填充车道面片。【F:csv2xodr/lane_spec.py†L463-L586】【F:csv2xodr/writer/xodr_writer.py†L235-L307】
...
- **宽度是“名义值”，白线是“实测值”。** lane link 给出的 359/391/363 cm 是设计/管理口径，我们会完整保留；白线点列展示现场真实情况。两套数据互补，而不是互相覆盖。【F:csv2xodr/lane_spec.py†L1120-L1183】【F:csv2xodr/writer/xodr_writer.py†L235-L307】
...
1. **先看 XODR 原文。** `out/JPN/map.xodr` 第 878–919 行如下，正是客户能看到的首个 `<laneSection>`：
   ```xml
   <laneSection s="0">
     <center>
   ...
   ```
   这段 XML 就是我们要拿给客户看的现成结果。【F:out/JPN/map.xodr†L878-L919】
2. **逐列对照 CSV 行号。** 下表把每个字段对应回原始 CSV 的确切行：

   | XODR 字段 | 数值 | CSV 来源行 | 说明 |
   | --- | --- | --- | --- |
   | `<lane id="1">` `width a=3.590` | 3.590 m | `PROFILETYPE_MPU_ZGM_LANE_LINK_INFO.csv` 第 2 行 `幅員情報[cm]=359` | 359 cm ÷ 100 = 3.59 m，`lane_spec` 把它写成 `a` 系数。【F:input_csv/JPN/PROFILETYPE_MPU_ZGM_LANE_LINK_INFO.csv†L1-L4】【F:csv2xodr/lane_spec.py†L463-L586】 |
   | `<lane id="2">` `width a=3.910` | 3.910 m | 同表第 3 行 `幅員情報[cm]=391` | 同上换算。【F:input_csv/JPN/PROFILETYPE_MPU_ZGM_LANE_LINK_INFO.csv†L1-L4】 |
   | `<lane id="3">` `width a=3.630` | 3.630 m | 同表第 4 行 `幅員情報[cm]=363` | 同上。【F:input_csv/JPN/PROFILETYPE_MPU_ZGM_LANE_LINK_INFO.csv†L1-L4】 |
   | `<lane id="4">` `width a=1.391` | 1.391 m | `PROFILETYPE_MPU_ZGM_SHOULDER_WIDTH.csv` 第 2–5 行 `Hl[cm]` 平均=139.0625 cm | `build_shoulder_profile` 求平均后除以 100。【F:input_csv/JPN/PROFILETYPE_MPU_ZGM_SHOULDER_WIDTH.csv†L1-L9】【F:csv2xodr/normalize/core.py†L1913-L1980】 |
   | `<lane id="-1">` `width a=0.398` | 0.398 m | 同一表第 6–9 行 `EHl[cm]` 平均=39.8125 cm | 同上。【F:input_csv/JPN/PROFILETYPE_MPU_ZGM_SHOULDER_WIDTH.csv†L1-L9】 |
   | `<roadMark type="solid" width="0.200">` | 0.200 m | `PROFILETYPE_MPU_ZGM_LANE_DIVISION_LINE.csv` 第 2–7 行 `始点側線幅[mm]=200` | 200 mm ÷ 1000 = 0.2 m，`mark_type_from_division_row` 识别为 solid。【F:input_csv/JPN/PROFILETYPE_MPU_ZGM_LANE_DIVISION_LINE.csv†L1-L7】【F:csv2xodr/lane_spec.py†L463-L586】 |
   | `<link><successor id="1/2/3/4">` | 对应 id | `PROFILETYPE_MPU_ZGM_LANE_LINK_INFO.csv` 第 1–4 行 `前方レーンID` | `build_lane_topology` 把这些指针写入 `link`。【F:input_csv/JPN/PROFILETYPE_MPU_ZGM_LANE_LINK_INFO.csv†L1-L4】【F:csv2xodr/topology/core.py†L156-L226】 |
   | `<laneSection s="0">` 长度 31.9456 m | 31.9456 | `PROFILETYPE_MPU_LINE_GEOMETRY.csv` 第 2–10 行 Offset=858733–861908 cm | `build_offset_mapper` 把 3175 cm 转成 31.9456 m。【F:input_csv/JPN/PROFILETYPE_MPU_LINE_GEOMETRY.csv†L1-L10】【F:csv2xodr/normalize/core.py†L462-L516】 |

3. **用大白话串起来。** 可以直接这样给客户解释：
   - “车道 1/2/3 的宽度，就是 lane link 表第 2/3/4 行的 359/391/363 cm，除以 100 就写进了 `<width>`。”
   - “左路肩 1.391 m、右路肩 0.398 m，是路肩宽度表同一段 139.0625 cm、39.8125 cm 的平均值。”
   - “白线 0.2 m 宽度来自区画线表的 200 mm。”
   - “`<link>` 里的 successor id 直接照抄 lane link 的 `前方レーンID`。”
   - “这一段的起止 Offset 是 858733–861908 cm，折算成米就是 31.9456 m 的 `<laneSection>` 长度。”

   这样就完成了客户要求的“指着现有 XODR，一一指出各字段来自哪个 CSV 的哪一行”。

## 17. 这段 XML 在查看器里的具体位置
- 上面引用的 `<laneSection s="0">` 覆盖的弧长是 `[0, 31.9456)` 米，对应 `<planView>` 里前九段 `<geometry>`。这些线段的坐标起点是 `x=-1.6884, y=-0.6345`，沿着道路由南向北略向左弯，如下所示。【F:out/JPN/map.xodr†L10-L38】
- 因此截图里的那条略向左弯的起始路段，正是这一段中心线及其左右 3 条行车道外加路肩。把查看器的 `s` 值置零或把相机拉到原点附近，就能看到图中那条灰色面片与白线重合的路段。【F:out/JPN/map.xodr†L878-L919】
- 向前（`s≈32 m`）即可进入下一段 `<laneSection s="31.94564674">`，它开始重新分配车道宽度。如果在查看器里沿着道路向前推进，看到灰色面片的分界发生变化，就是从第一个 laneSection 跨到第二个 laneSection 的标志。【F:out/JPN/map.xodr†L919-L958】
